...
Proof.
  ...
  apply A_imp_B_imp_C in A_imp_B; assumption.
Qed.
...
Proof.  intros proof_of_False. reflexivity. Qed.
...
Proof.  unfold not. intros. destruct H0. assumption. Qed.
...
Proof.  intros A B proof_of_A proof_of_B.  split; assumption. Qed.

Theorem and_commutes : (forall A B :Prop, A/\B -> B/\A).
Proof.  intros A B H. destruct H as [Ha Hb].  split; assumption. Qed.

Theorem orb_is_or : (forall a b:bool, Is_true (orb a b) <-> Is_true a \/ Is_true b).
Proof.  intros a b. destruct a,b; intuition. Qed.

Theorem andb_is_and : (forall a b:bool, Is_true (andb a b) <-> Is_true a /\ Is_true b).
Proof.  intros.   destruct a,b; intuition. Qed.
...
Proof.  unfold basic_predicate. exists true. reflexivity. Qed.

Theorem thm_forall_exists : (forall b:bool, (exists a:bool, Is_true(eqb a b))).
Proof.  intros. exists b. destruct b; reflexivity. Qed.
...
Proof. unfold not. intros. destruct H0. apply (H x). assumption. Qed.
...
Proof. destruct a; intuition. Qed.
...
Proof. induction n as [|n']; simpl; try (rewrite -> IHn'); reflexivity. Qed.

Theorem plus_sym : (forall n m, n+m=m+n).
Proof.
  intros n. induction n as [|n']; intros; simpl.
  rewrite -> (plus_n_O m). reflexivity.
  rewrite -> IHn'.
  induction m as [|m']; simpl; [|rewrite -> IHm']; reflexivity.
Qed.
...
Proof.  intros A. intros. split; reflexivity. Qed.
...
Proof. intros. induction l as [|h l']; simpl; try (rewrite -> IHl'); reflexivity. Qed.

Theorem app_comm_cons : forall A (x y:list A) (a:A), a :: (x ++ y) = (a :: x) ++ y.
Proof. intros A x y. induction x as [|h x']; intros; simpl; try (rewrite -> (IHx' h)); reflexivity. Qed.

Theorem app_assoc : forall A (l m n:list A), l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n.
  induction l as [|h l']; simpl; try (rewrite -> IHl'); reflexivity.
Qed.

Theorem app_cons_not_nil : forall A (x y:list A) (a:A), nil <> x ++ a :: y.
Proof. intros. unfold not. induction x as [|h l']; simpl; intros; inversion H. Qed.
